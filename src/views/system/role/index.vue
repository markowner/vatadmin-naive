<template>
  ...
  <Permission ref="permissionForm" @submit="toSubmitComplete"></Permission>
</template>
<script setup>
          ...
          return [...defaultColumn, h(NButton,
              {
                ...
                secondary: true,
                onClick: () => {
                  permissionForm.value.injectData({role_id: row.id}).show()
                }
              },
              {default: () => '设置权限'}
          )]
  ...
  function toolsChange(type, event, ids){
    switch(type){
      ...
      case 'download':
        ...
        Request.request(pageJsonData.api_list.download, {filter: JSON.stringify(vPage.value.state.params)}, {responseType: 'blob'}).then(res => {
          tools.notice.message.success(res.msg)
        }).catch(err => {
          console.log(err)
        })
